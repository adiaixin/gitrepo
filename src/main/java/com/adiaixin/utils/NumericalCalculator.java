package com.adiaixin.utils;
import com.adiaixin.enums.EmOperator;
...
public class NumericalCalculator {
    ...
    public static double cal(String expression, String... values)  {
        ...
        BigDecimal result =  _cal(expression, opStack, dataStack);
     ...
     * 计算主逻辑
     */
    private static BigDecimal _cal(String expression, Stack<EmOperator> opStack, Stack<BigDecimal> dataStack) {
        ...
        for (int i = 0; i < length; ) {
            ...
            if (Character.isDigit(value) || NEGATIVE_SIGN == value) {
                ...
                if (null == curOp) {
                    throw new RuntimeException( "未知的操作符:" + strBuilder.toString());
                }
                if (charArr[i] != '(') { throw new RuntimeException( "公式[" + curOp + "]格式不正确，缺少‘('");}
                ...
                if (rightIndex < 0) { throw new RuntimeException( "公式[" + curOp + "]格式不正确，缺少‘)'");}
                ...
                if (params.size() != curOp.getParamNumber()){ throw new RuntimeException( "公式，参数个数不正确:[运算符号:" + curOp + "][需要参数个数:" + curOp.getParamNumber() + "][入参:" + expression + "]");}
                for (String param : params) {
                    dataStack.push(new BigDecimal(NumericalCalculator.cal(param)).setScale(20, RoundingMode.HALF_UP));
                }
                ...
                throw new RuntimeException( "未知字符:" + String.valueOf(value));
            }
        }
    ...
    private static void singleCal(Stack<EmOperator> opStack, Stack<BigDecimal> dataStack) {
        ...
        switch (op) {
            ...
            case MULTIPLY:
                ...
                break;
            case DIVIDE:
                if (BigDecimal.ZERO.equals(firstValue) || BigDecimal.ZERO.equals(secondValue)) {
                    result = BigDecimal.ZERO.doubleValue();
                } else {
                    result = firstValue.divide(secondValue, 4, RoundingMode.HALF_UP).doubleValue();
                }
                break;
            case ROUND :
                result = Math.round(firstValue.doubleValue());
                break;
            case MOD:
            ...
            case POW:
                ...
                break;
            case SQRT:
                if (firstValue.doubleValue() < 0) { throw new RuntimeException( "SQRT公式参数小于零:" + firstValue);}
            ...
            case LOG10:
                ...
                break;
            default:
                throw new RuntimeException( "未实现的运算符[" + op + "]");
        }
        dataStack.push(new BigDecimal("" + result).setScale(20, RoundingMode.HALF_UP));
        return;
    }
     ...
     * @return
     */
    private static String preProcess(String expression, String... values)  {
        ...
        if (values.length > 0) {
            int paramIndex = 0;
            int $Index = 0;
            for (; $Index < expression.length() && ($Index = expression.indexOf(EXPRESSION_PLACEHOLDER, $Index)) >= 0; ++paramIndex) {
                expression = expression.substring(0, $Index) + new BigDecimal(values[paramIndex]) + expression.substring($Index + 1);
            }
            if (values.length != paramIndex || $Index >= 0){throw new RuntimeException( "占位符个数与参数个数不符");}
        }
        ...
        if (arr[0] == '*' || arr[0] == '/' || arr[0] == '%' || arr[arr.length - 1] == '*' || arr[arr.length - 1] == '/' || arr[arr.length - 1] == '%') { throw new RuntimeException( "表达式格式不正确,不能以*,/,%开头或结尾");}
    ...
    public static List<BigDecimal> cal4Arr(String expression, List<BigDecimal>... params) {
        if (params.length < 1){ throw new RuntimeException( "参数数目不正确[" + expression + "][params=" + params + "]");}
        int paramLen = params[0].size();
        for (List<BigDecimal> valueList : params) {
            if (paramLen != valueList.size()) {  throw new RuntimeException( "参数列表长度不相等[" + expression + "][params=" + params + "]");}
        }
        ...
        if (paramsCount != params.length){ throw new RuntimeException( "参数数目不正确[" + expression + "][params=" + params + "]");}
        List<BigDecimal> resultList = new ArrayList<BigDecimal>();
        for (int i = 0; i < paramLen; ++i) {
            String[] singleParams = new String[params.length];
            for (int j = 0; j < params.length; ++j) {
                singleParams[j] = params[j].get(i).toString();
            }
    ...
    public static void main(String[] arr) {
        try {
            ...
            System.out.println(NumericalCalculator.cal("((12.3 - 2.3 * 1) * ((5 + 3) / 1))"));
            System.out.println(NumericalCalculator.cal("-pow(min(1 + 2, 2),-1 * abs(max(1,1/-4+1/4)))"));
            //占位符方式
            System.out.println(NumericalCalculator.cal("pow($,$) * abs($)", "2", "4", "-3"));
            System.out.println(NumericalCalculator.cal("log(exp(10))"));
            ...
            param3List.add(new BigDecimal("2"));
            System.out.println(NumericalCalculator.cal4Arr("pow($,$) * exp($)", param1List, param2List, param3List));

            System.out.println(NumericalCalculator.cal("pow($,$) * abs($)", "2", "4", "-3", "1"));//参数异常Test
        } catch (Exception efEx){
            efEx.printStackTrace();
        }
